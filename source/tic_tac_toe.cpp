...
internal inline ttt_mark
TTT2DBoardTestWin(tic_tac_toe_state *State, ttt_board *Board){
 TIMED_FUNCTION();
...
internal inline ttt_mark
TTT3DBoardTestWin(tic_tac_toe_state *State, ttt_board *Board){
 TIMED_FUNCTION();
...
internal inline ttt_mark
TTT4DBoardTestWin(tic_tac_toe_state *State, ttt_board *Board){
 TIMED_FUNCTION();
...
UpdateAndRenderTicTacToe(){
 ...
 switch(Board.Type){
  case TTTBoard_2D: {
   ...
   Winner = TTT2DBoardTestWin(State, &Board);
  }break;
  case TTTBoard_3D: {
   TTT3DBoardRender(State, &Board, State->P, Padding, 5.0f);
   Winner = TTT3DBoardTestWin(State, &Board);
  }break;
  case TTTBoard_4D: {
   TTT4DBoardRender(State, &Board, State->P, Padding, 5.0f);
   Winner = TTT4DBoardTestWin(State, &Board);
  }break;
 }
